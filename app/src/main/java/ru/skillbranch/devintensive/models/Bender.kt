...
//    var cntWrongAns = 0
    ...
    fun listenAnswer(answer:String):Pair<String, Triple<Int, Int, Int>> {
        val validateResult = question.validate(answer)
        ...
        return if (question.answers.contains(answer.toLowerCase())){
            ...
            if (question.name != "IDLE") "Отлично - ты справился\n${question.question}" to status.color
    ...
    enum class Question(val question: String, val answers:List<String>) {
            ...
            override fun validate(answer: String): String? {
                if (!answer.isBlank()) {
                    for (c:Char in answer) {
                        if (c.toString().toIntOrNull()!= null) return "Материал не должен содержать цифр"
                    }
                }
            return null
            }
        ...
        SERIAL("Мой серийный номер?", listOf("2716057")){
            ...
            override fun validate(answer: String): String? =
                if (!answer.isBlank() && (answer.toIntOrNull() == null || answer.length != 7))
